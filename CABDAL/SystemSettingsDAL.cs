...
namespace CAB.DALC.Data
{
    public class SystemSettingsDAL : DALBase
    {
        ...
        private static readonly IGeneralLog logger = LogFactory.CreateGeneralLogger(typeof(SystemSettingsDAL).ToString());
        public void InsertSystemSettings()
        {
            string[] qry = new string[4];
            //qry[0] = "Insert Into metertype_master(MeterType_Name) values('1P3W')";
            qry[0] = "Insert Into dlms_ltct_650.system_settings(Name,Value) values('CMRI_COM_PORT','COM1')";
            qry[1] = "Insert Into dlms_ltct_650.system_settings(Name,Value) values('GSM_COM_PORTS','COM1')";
            //qry[1] = "Insert Into system_settings(Name,Value) values('GSM_COM_PORTS','COM2,COM3,COM4,COM5,COM6')";
            qry[2] = "Insert Into dlms_ltct_650.system_settings(Name,Value) values('USE_MULTIPLE_PORTS','1')";
            qry[3] = "Insert Into dlms_ltct_650.system_settings(Name,Value) values('COM_PORT','COM1')";
        ...
        /// <summary>
        /// Datalayer method to insert report column master data
        /// </summary>
        /// <returns></returns>
        public void InsertGPRSSchedulingReportColumns(string xml)
        {
            try
            {
                ...
                builder.Append("`dlms_ltct_650`.InsertSchedulingColumns");
                DataRequest request = new DataRequest(builder.ToString(), CommandType.StoredProcedure);
                request.AddParamter(ParameterName("xml"),xml, DbType.Xml);
                helper.FillDataSet(request, ds);

            }
            catch (CABException ex)    //Exception log for catch block
            {
                logger.Log(LOGLEVELS.Error, "InsertGPRSSchedulingReportColumns(string xml)", ex);
            }
        ...
        public string GetSettingValue(string name)
        {
            ...
            {
                logger.Log(LOGLEVELS.Error, "GetSettingValue(string name)", ex);
