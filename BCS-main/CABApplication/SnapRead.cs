...
namespace CABApplication
{
    ...
    /// </summary>
    public partial class SnapRead : MdiChildForm
    {
        #region Nested Types
        #endregion
        ...
        private void GenerateSnapRead(object state)
        {
            ...
            try
            {
                ...
                List<ProfileCommand> profileReadCommands = GetProfileCommandsToRead(lstProfileCommands, ProfileId.Instant, NamePlateConstants.PumaLTE650Value);
                        ...
                        if (result.RecieveDataBuffer != null && result.RecieveDataBuffer.Count > 0)
                        {
                            ...
                            while (true)
                            {
                                ...
                                try
                                {
                                    ...
                                    {
                                        this.StatusMessageAsync = "Snap Read readout stopped.";
                                    }
                                }
                                catch (Exception)
                                {
        ...
        private void ConvertInstantEntityToDataSet(List<DLMS650InstantaneousEntity> instantEntityList)
        {
            ...
            foreach (DLMS650InstantaneousEntity instantEntity in instantEntityList)
            {
                ...
                if (instantEntity.InstantPowerColumnName.ToUpper().Contains("DATE") || instantEntity.InstantPowerColumnName.ToUpper().Contains("TIME"))
                {
                    if (counter == 1)
                    {
                        dateValue = DateUtility.LongToStringDateTimeWithSecFormat(Int64.Parse(instantEntity.InstantPowerColumnValue));
                        instantRow["Unit"] = string.Concat(ConfigInfo.DateFormat(), " HH:MM:SS");
                        dateValue = dateValue.Replace("99", "00");
                        instantRow["Value"] = dateValue;
                    }
                    else if (instantEntity.InstantPowerColumnName.ToUpper().Contains("ELAPSED"))
                    {
                        string[] data = instantEntity.InstantPowerColumnValue.Split('*');
                        DateTime epochTime = new DateTime(1970, 1, 1, 0, 0, 0);
                        TimeSpan timeSpan = DateUtility.LongToDateTime(Int64.Parse(data[0])) - epochTime;
                        instantRow["Value"] = ConvertTimSpanToMMSS(timeSpan);
                        instantRow["Unit"] = DateFormatForElapsedTime;
                    }
                    else
                    {
                        dateValue = DateUtility.LongToStringDateTimeFormat(Int64.Parse(instantEntity.InstantPowerColumnValue));
                        instantRow["Unit"] = string.Concat(ConfigInfo.DateFormat(), " HH:MM");
                        dateValue = dateValue.Replace("99", "00");
                        instantRow["Value"] = dateValue;
                    }
                ...
                else
                {
                    ...
                    if (instantEntity.InstantPowerColumnName.Contains(CumPowerFailureForInstant))
                    {
                        string response = ConfigInfo.SignatureInfo;


                        TimeSpan timeSpan = TimeSpan.FromMinutes(Convert.ToDouble(dataValue[0]));

                        if (chkPowerOnOffDurationFormat == "1")
                        {
                            instantRow["Value"] = new DLMS650CommonBLL().ConvertTimSpanToDDDDHH(timeSpan);
                            instantRow["Unit"] = DateFormatForInScumPowFailDDDDHH;
                        }
                        else
                        {
                            instantRow["Value"] = new DLMS650CommonBLL().ConvertTimSpanToDDHHMM(timeSpan);
                            instantRow["Unit"] = DateFormatForInScumPowFail;
                        }
                        ...
                        //Date- 22/04/2016 Forcely we have remove the "Cumulative Power-Failure Duration" for meter model no "SK" and "VB" as it was coming a garbage value from the 1P DLMS VIM meter. Bug Id: 454033
                        //******* Meter Model Change Required Here ***********//
                        if (!response.Contains("SK") && !response.Contains("VB") && !response.Contains("FS") && !response.Contains("VF") && !response.Contains("SF"))
                        {
                    ...
                    if ((instantEntity.InstantPowerColumnName.Contains(CUMPOTENTIALFAILRPHASE)) || (instantEntity.InstantPowerColumnName.Contains(CUMPOTENTIALFAILYPHASE)) || (instantEntity.InstantPowerColumnName.Contains(CUMPOTENTIALFAILBPHASE)) || (instantEntity.InstantPowerColumnName.Contains(CUMCURRENTFAILRPHASE)) || (instantEntity.InstantPowerColumnName.Contains(CUMCURRENTFAILYPHASE)) || (instantEntity.InstantPowerColumnName.Contains(CUMCURRENTFAILBPHASE)))
                    {
                        TimeSpan timeSpan = TimeSpan.FromMinutes(Convert.ToDouble(dataValue[0]));
                    ...
                    {
                        instantRow["Value"] = (Convert.ToDecimal(dataValue[0])).ToString();
                    }
                    else
                    {
