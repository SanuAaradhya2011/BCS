...
class Program
{
    static void Main()
    {
        string path = @"C:\Users\PIYUSH SINGH\Downloads\3PhaseBCS\3PhaseBCS\CABApplication\LogDetails.cs";
        ...
        {
            var grid = grdActivityLogDetails.Controls.Find(""grdData"", true)[0] as DataGridView;
            if (grid == null) return;
            
            grid.ColumnHeadersVisible = false;
            grid.RowHeadersVisible = false;
            grid.CellBorderStyle = DataGridViewCellBorderStyle.None;
            grid.BackgroundColor = Color.FromArgb(245, 247, 250);
            grid.DefaultCellStyle.BackColor = Color.FromArgb(245, 247, 250);
            grid.DefaultCellStyle.SelectionBackColor = Color.FromArgb(245, 247, 250);
            grid.DefaultCellStyle.SelectionForeColor = Color.Black;
            grid.AllowUserToResizeRows = false;
            grid.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            
            grid.RowPrePaint -= GrdActivityLogDetails_RowPrePaint;
            grid.RowPrePaint += GrdActivityLogDetails_RowPrePaint;
            grid.RowPostPaint -= GrdActivityLogDetails_RowPostPaint;
            grid.RowPostPaint += GrdActivityLogDetails_RowPostPaint;
            grid.DataBindingComplete -= GrdActivityLogDetails_DataBindingComplete;
            grid.DataBindingComplete += GrdActivityLogDetails_DataBindingComplete;
        }
        ...
        private void GrdActivityLogDetails_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            ...
            UpdateTopHeaderPanel(grid);
        }

        private Panel topHeaderPanel;
        private void UpdateTopHeaderPanel(DataGridView grid)
        {
            if (topHeaderPanel == null)
            {
                ...
                topHeaderPanel.Paint += (s, e) => {
                    ...
                    if (grid.Rows.Count > 0 && grid.Rows[0].DataBoundItem is DataRowView firstDrv)
                    {
                ...
                // Add it above the grid inside the form
        ...
        {
            var grid = sender as DataGridView;
            if (!(grid.Rows[e.RowIndex].DataBoundItem is DataRowView drv)) return;
            ...
            string tag = grid.Rows[e.RowIndex].Tag as string;
