...
namespace CAB.UI
{
	public partial class ExportMeterData : MdiChildForm
	{
				...
				for (int Countlist = 0; Countlist < columnList.Count; Countlist++)
				{
					foreach (PropertyInfo pInfo in props)
					{
						if (pInfo.Name.Contains(columnList[Countlist].ToString()))
						{
							...
							{
								value = Convert.ToString(pInfo.GetValue(entity, new object[] { }));
								file.Write(GetASCIIValue(value));
							}
							else
							{
