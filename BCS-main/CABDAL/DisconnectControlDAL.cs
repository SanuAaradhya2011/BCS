...
namespace CABDAL
{
    public class DisconnectControlDAL : DALBase
    {
        private string MeterDataID = "MeterData_ID";
        private string DCData = "DCData";
        private static readonly IGeneralLog logger = LogFactory.CreateGeneralLogger(typeof(DisconnectControlDAL).ToString());

        public override IEntity InsertData(IEntity entity)
        {
            DisconnectControlEntity dcEntity = entity as DisconnectControlEntity;
            try
            {
                IDataHelper helper = DatabaseFactory.GetHelper();
                StringBuilder builder = new StringBuilder();
                builder.Append("Insert into disconnectcontrol(MeterData_ID,DCData) values(");
        ...
        /// <returns></returns>
        public DisconnectControlEntity GetData(Int64 MeterData_ID)
        {
            DisconnectControlEntity DCEntity = new DisconnectControlEntity();
            try
            {
                IDataHelper helper = DatabaseFactory.GetHelper();
                DataRequest request = new DataRequest("select IFNULL(DCData,'') from disconnectcontrol where  MeterData_ID=" + MeterData_ID);
                object result = helper.ExecuteScalar(request);
                DCEntity.DCData = result == null ? string.Empty : result.ToString();
                UserLogActivityDAL.GetInstance().GenerateAndSaveLog(string.Concat("Disconnect Control read from DB"));
            }
        ...
        public bool DeleteData(long meterDataID)
        {
            ...
            try
            {
                IDataHelper helper = DatabaseFactory.GetHelper();
                StringBuilder builder = new StringBuilder();
                builder.Append("Delete from disconnectcontrol where ");
                ...
                helper.ExecuteNonQuery(request);
                UserLogActivityDAL.GetInstance().GenerateAndSaveLog(string.Concat("disconnectcontrol data for a specified file deleted."));
                Flag = true;
            }
            catch (Exception ex)    //Exception log for catch block
            {
