...
{
    public class MidnightParameterDAL : DALBase
    {
        private string ColumnsNames = "ColumnsNames";
        private string MeterData_ID = "MeterData_ID";
        private string MeterID = "MeterID";
        private static readonly IGeneralLog logger = LogFactory.CreateGeneralLogger(typeof(MidnightParameterDAL).ToString());

        public override IEntity InsertData(IEntity entity)
        {
            DTMDailyProfileParameterEntity midnightParameterEntity = null;
            if (entity == null)
                return midnightParameterEntity;
            midnightParameterEntity = entity as DTMDailyProfileParameterEntity;
            bool Flag = false;
            try
            {
                IDataHelper helper = DatabaseFactory.GetHelper();
                StringBuilder builder = new StringBuilder();
                builder.Append("Insert Into midnightparameter(ColumnsNames,MeterData_ID) values(");
                builder.Append(string.Concat(ParameterName(ColumnsNames), ","));
                builder.Append(string.Concat(ParameterName(MeterData_ID), ")"));
                DataRequest request = new DataRequest(builder.ToString());
                request.AddParamter(ParameterName(ColumnsNames), midnightParameterEntity.ColumnsNames, DbType.String, 1000);
                request.AddParamter(ParameterName(MeterData_ID), midnightParameterEntity.MeterDataId, DbType.Int64);
                helper.ExecuteNonQuery(request);
                UserLogActivityDAL.GetInstance().GenerateAndSaveLog(string.Concat("Midnight data added"));
                Flag = true;
            }
            catch (CABException ex)    //Exception log for catch block
            {
                logger.Log(LOGLEVELS.Error, "InsertData(IEntity entity)", ex);
                midnightParameterEntity = null;
            }
            return midnightParameterEntity;
        }
        ...
        public bool DeleteData(long meterDataID)
        {
            ...
            try
            {
                IDataHelper helper = DatabaseFactory.GetHelper();
                StringBuilder builder = new StringBuilder();
                builder.Append("Delete from midnightparameter where ");
        ...
        public IEntity GetDetailData(long id)
        {
            ...
            try
            {
                IDataHelper helper = DatabaseFactory.GetHelper();
                StringBuilder builder = new StringBuilder();
                builder.Append("Select ColumnsNames,MeterData_ID from midnightparameter where ");
                builder.Append(string.Concat(MeterData_ID, "=", ParameterName(MeterData_ID)));
                DataRequest request = new DataRequest(builder.ToString());
                ...
                if (ds.Tables!= null && ds.Tables.Count > 0 &&  ds.Tables[0].Rows.Count > 0)
                    midnightParameterEntity = (DTMDailyProfileParameterEntity)RowToEntity(ds.Tables[0].Rows[0]);
        ...
        public DataSet GetColumnNamesForMeterID(string meterID)
        {
            ...
            try
            {
                IDataHelper helper = DatabaseFactory.GetHelper();
                StringBuilder builder = new StringBuilder();
                builder.Append("SELECT distinct parameter.ColumnsNames  FROM midnightparameter parameter , meterdata mdata");
        ...
        public override IEntity RowToEntity(DataRow row)
        {
            ...
            if (NotNullAndNotDBNull(row, MeterData_ID)) midnightParameterEntity.MeterDataId = Convert.ToInt32(row[MeterData_ID]);
            if (NotNullAndNotDBNull(row, ColumnsNames)) midnightParameterEntity.ColumnsNames = Convert.ToString(row[ColumnsNames]);
            return midnightParameterEntity;
        }
