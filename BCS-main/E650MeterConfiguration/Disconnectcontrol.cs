...
namespace CAB.E650MeterConfiguration
{
    ...
    /// </summary>
    public class Disconnectcontrol:BaseConfig
    {
        #region Nested Types
        #endregion

        #region Constants and Variables
        private static readonly IGeneralLog logger = LogFactory.CreateGeneralLogger(typeof(Disconnectcontrol).ToString());
        #endregion

        #region Properties
        public List<byte> InputData { get; 
            set; 
        }
        #endregion

        #region Constructor
        public Disconnectcontrol(bool isLittleEndian)
            : base(isLittleEndian)
        { }
        #endregion
        ...
        private List<MeterDataPacket> GetMeterDataPackets(DLMSCOMMAND dlmsCommand, ExtendedBinaryReader reader)
        {
            ...
            try
            {
                ...
                for (int counter = 0; counter < binaryData.Length; counter++)
                {
