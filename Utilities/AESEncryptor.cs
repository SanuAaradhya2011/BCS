...
/* 
 * |----------------------------------------------------------------------------------------------------------------|
 * |											AESEncryptor class for AES GCM 128 Encryption  						|
 * |											Author : Piyush Singh   									    |
 * |											Date   : 17-Apr-2013			     								|
 * | 																												|
 * |----------------------------------------------------------------------------------------------------------------| */
...
namespace Utilities
{
    ...
    0x61, 0xc2, 0x9f, 0x25, 0x4a, 0x94, 0x33, 0x66, 0xcc, 0x83, 0x1d, 0x3a, 0x74, 0xe8, 0xcb  };
        ...
        public byte[] fAESEncryption(string hLSKey, byte[] rbuffer)
        {
            int i = 0;
            ...
            byte[] HLSKey = new byte[hLSKey.Length];

            int charCounter = 0;
            foreach(char keyChar in hLSKey.ToCharArray())
            {
                HLSKey[charCounter] = (byte)keyChar;
                charCounter++;
            }
            Nr = 128;
            ...
            Nk = Nr / 32;
            Nr = Nk + 6;
            //fFlashRead(temp,BASE_ADD_SECRET_KEY,BASE_ADD_SECRET_KEY_LENGTH);
            byte[] recvRdmKey = new byte[16];
            int j = 0;
            for (i = 0, j = 0; i < 16; )
            {
                recvRdmKey[i++] = rbuffer[j++];
            }
            i = 0;
            while (i < 16)
            {
