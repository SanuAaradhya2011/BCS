...
namespace CAB.UI
{
    public partial class FolderSelectDialog : Form
    {
        ...
        private static readonly IGeneralLog logger = LogFactory.CreateGeneralLogger(typeof(FolderSelectDialog).ToString());
        ...
        private void fillTree()
        {
            ...
            foreach (char c in driveLetters)
            {
                ...
                catch (Exception doh)    //Exception log for catch block
                {
                    Console.WriteLine(doh.Message);
                    logger.Log(LOGLEVELS.Error, "fillTree()", doh);
                }
            }
        }
        ...
        private void getSubDirs(TreeNode parent)
        {
            ...
            {
                Console.WriteLine(doh.Message);
                logger.Log(LOGLEVELS.Error, "getSubDirs(TreeNode parent)", doh);
            }
        }
        private string fixPath(TreeNode node)
        {
            ...
            {
                Console.WriteLine(doh.Message);
                logger.Log(LOGLEVELS.Error, "fixPath(TreeNode node)", doh);
            }
            return sRet;
        }
