...
namespace CAB.UI
{
	public partial class NewGroupDefinition : MdiChildForm
	{
        ...
        private static readonly IGeneralLog logger = LogFactory.CreateGeneralLogger(typeof(NewGroupDefinition).ToString());
		...
		private void lngButton1_Click(object sender, EventArgs e)
		{
			...
			{
                logger.Log(LOGLEVELS.Error, "lngButton1_Click(object sender, EventArgs e)", ex);
			}
		}
		...
		private void lngButton3_Click(object sender, EventArgs e)
		{
			...
			{
                logger.Log(LOGLEVELS.Error, "lngButton3_Click(object sender, EventArgs e)", ex);
			}
		}
		...
		private void lngButton4_Click(object sender, EventArgs e)
		{
			...
			{
                logger.Log(LOGLEVELS.Error, "lngButton4_Click(object sender, EventArgs e)", ex);
			}
		}
		...
		private void btnSave_Click(object sender, EventArgs e)
		{
			...
			else
			{
                ...
                else if (!ValidationProvider.ValidateData(subGroupName, ValidationConstant.groupName))
                {
                    MessageBox.Show("SubGroup Name does not accept special Characters", "BCS", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
		...
		private void NewGroupDefinition_Load(object sender, EventArgs e)
		{
			...
			foreach (DataRow dr in ds.Tables[0].Rows)
			{
                ...
                listBoxSelectedMeters.Items.Add(data);
                if (listBoxAvailableMeters.Items.Count > 0)
                {
                    for (int i = 0; i < listBoxAvailableMeters.Items.Count; i++)
                    {
                        if (listBoxAvailableMeters.Items[i].ToString() == data)
                            listBoxAvailableMeters.Items.Remove(data);
                    }
                }
			}
