...
namespace SerialCommunication
{
    public class SerialComm
    {
        ...
        /// <summary>
        /// Gets or sets IMEI number
        /// </summary>
        public string ImeiNumber
        {
            get { return modemImeiNumber; }
            set { modemImeiNumber = value; }
        }
        ...
        public bool fSendDataToPort(byte[] Buffer, byte nLength)
        {
            ...
            //If Selected communication type is GPRS then pass the buffer to GPRS Tunnel
            //Else pass it on to Com PORT 
            if (string.Equals(communicationMode, "GPRS", StringComparison.OrdinalIgnoreCase))
            {
                byte[] command = new byte[nLength];
                for (int i = 0; i < nLength; i++)
                {
                    command[i] = Buffer[i];
                }
                result = GPRSCommunication(command, ref ReceiveBuffer);                
            }
            else
            {
                for (int i = 0; i < 255; i++)
               ...
               result = SerialCommunication(Buffer, nLength);
            }
        ...
        /// <summary>
        /// 1.Method Returns true if Command byte sent to M2M successfully and Response is recieved from 
        /// M2M Server.
        /// 2. Method also returns the Response bytes received from the M2M.
        /// </summary>
        /// <param name="Buffer"></param>
        /// <param name="ReceiveBuffer"></param>
        /// <returns></returns>
        private bool GPRSCommunication(byte[] Buffer, ref byte[] ReceiveBuffer)
        {
            bool result = false;
        ...
        private bool SerialCommunication(byte[] Buffer, byte nLength)
        {
        ...
        public void SetSerialPortSettings(string commType)
        {
            communicationMode = commType;
        }
