    ...
    public partial class DisplayParamaters : UserControl
    {
       ...
       /// </summary> 
        public DisplayParamaters()
        {
            ...
            int value = new DLMS650GeneralBLL().GetMeterDisplayProgrammingVariantByMeterDataID(ConfigInfo.ActiveMeterDataId);
            if (value == (int)DisplayProgrammingTypes.TwoByte)
            {
                ConfigInfo.DisplayProgrammingVariant = DisplayProgrammingTypes.TwoByte;
            }
            else
            {
                ConfigInfo.DisplayProgrammingVariant = DisplayProgrammingTypes.OneByte;
            }
            if (dgridPushDisplayParams.Rows.Count == 0)
                if (MeterModelNumber == NamePlateConstants.SmartM_Cipher_HTCT || MeterModelNumber == NamePlateConstants.SmartM_Cipher_LTCT || MeterModelNumber == NamePlateConstants.SmartM_Cipher_WCM)
                {
                    FillDisplayParametersforFalcon(MeterModelNumber);
                }
                else if (MeterModelNumber == NamePlateConstants.SmartM_Cipher_1PH)
                {
                    FillDisplayParametersforFalcon(MeterModelNumber);
                }
                else
        ...
        /// Binding Paramter list to various datagrids.
        /// </summary>
        public void FillDisplayParameters(bool flag)
        {            
            ...
            {//Read the Parameters from the XML file.
                ...
                {
                    xmlDatadoc.DataSet.ReadXml(string.Concat(Path.GetDirectoryName(Application.ExecutablePath), "\\", "DisplayParameters_Extended.xml"));  
                }
                else if (flag)
                {
                    xmlDatadoc.DataSet.ReadXml(string.Concat(Path.GetDirectoryName(Application.ExecutablePath), "\\", "LTCT_DisplayParameters.xml"));          
                }
                else
                {
        ...
        #endregion
        private void FillDisplayParametersforFalcon(int MeterModelNumber)
        {
            XmlDataDocument xmlDatadoc = null;
            DataSet ds = null;
            try
            {//Read the Parameters from the XML file.
                ...
                if (MeterModelNumber == NamePlateConstants.SmartM_Cipher_HTCT || MeterModelNumber == NamePlateConstants.SmartM_Cipher_LTCT || MeterModelNumber == NamePlateConstants.SmartM_Cipher_WCM)
                {
                    xmlDatadoc.DataSet.ReadXml(string.Concat(Path.GetDirectoryName(Application.ExecutablePath), "\\", "DisplayParameters_Falcon3PH.xml"));
                }
                else if (MeterModelNumber == NamePlateConstants.SmartM_Cipher_1PH)
                {
                    xmlDatadoc.DataSet.ReadXml(string.Concat(Path.GetDirectoryName(Application.ExecutablePath), "\\", "DisplayParameters_Falcon1PH.xml"));
                }
